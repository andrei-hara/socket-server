...
//            loc = in.readUTF();
          ...
          int status = con.getResponseCode();
            ...
            //preluare val woeid
            JsonNode result = node.get(0).get("woeid");

            //HTTP request pentru preluare date meteo(PT 2 DE COD)
            URL url1 = new URL("https://www.metaweather.com/api/location/" + URLEncoder.encode(String.valueOf(result), String.valueOf(StandardCharsets.UTF_8)) + "/");
            ...
            JsonNode result1 = node1.get("consolidated_weather").get(0).get("weather_state_name");
            JsonNode result2 = node1.get("consolidated_weather").get(0).get("the_temp");
            JsonNode result3 = node1.get("consolidated_weather").get(0).get("min_temp");
            JsonNode result4 = node1.get("consolidated_weather").get(0).get("max_temp");

            //afisare informatii
            out.writeUTF("weather state: " + result1.asText() + " current temperature: " + result2.asText() + " lowest temp: " + result3.asText() + " highest temp: "+ result4.asText());
